...
var MiniProfiler = (function () {
    ...
    var initPopupView = function () {
        ...
        var getIds = function (stringIds) {
            if (/\],\s?\[/.test(stringIds)) {
                var index = stringIds.indexOf("], [");
                if (index < 0) {
                    index = stringIds.indexOf("],[");
                }
                stringIds = stringIds.substr(0, index + 1);
            }
            return typeof JSON != 'undefined' ? JSON.parse(stringIds) : eval(stringIds);
        };
